-- ============================================================
-- Supabase: 통합 마이그레이션 v2 (정리·통합) - Clean
...
COMMENT ON COLUMN items.total_quantity IS '총 수량. 표준 단위 기준. 있으면 단위당 원가 = price/total_quantity';
COMMENT ON COLUMN items.order_disabled IS 'true이면 매장 발주 품목 검색에 노출되지 않음';
...
-- warehouse_locations (supabase_schema에 있을 수 있음)
...
ALTER TABLE petty_cash_transactions ADD COLUMN IF NOT EXISTS account_subject_id BIGINT NULL;
CREATE INDEX IF NOT EXISTS idx_petty_cash_account_subject ON petty_cash_transactions(account_subject_id);
COMMENT ON COLUMN petty_cash_transactions.account_subject_id IS '계정과목 ID - account_subjects 참조';
...
COMMENT ON COLUMN bank_transactions.invoice_received IS '인보이스 수령 여부';
...
COMMENT ON COLUMN bank_transactions.purchase_order_id IS '연동된 발주서 ID';
COMMENT ON COLUMN bank_transactions.category IS 'transfer=이체, expense=비용, fixed=고정비, correction=정정';
...
-- pos_menus code NULL 방지 (NOT NULL 제약 위반 해결)
...
DROP POLICY IF EXISTS "Allow all for pos_settlements" ON pos_settlements;
CREATE POLICY "Allow all for pos_settlements" ON pos_settlements FOR ALL USING (true) WITH CHECK (true);
