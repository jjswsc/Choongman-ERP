...
# 각 항목: 바이트 배열. WritePrinter 성공 시 첫 시퀀스만 전송되므로 제조사 호환 순서가 중요.
# Zywell(ZY307 등) 매뉴얼: ESC d 7 + GS V 0/1 (1B 64 07 1D 56 00|01). ESC @ 선행은 이중 절단·빈 토출 유발 가능 → 뒤로.
$variants = @(
  [byte[]]@(0x1B, 0x64, 0x07, 0x1D, 0x56, 0x00),
  [byte[]]@(0x1B, 0x64, 0x07, 0x1D, 0x56, 0x01),
  [byte[]]@(0x1B, 0x64, 0x07, 0x1D, 0x56, 0x42, 0x00),
