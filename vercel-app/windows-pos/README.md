...
- **첫 실행 시**: `userData` 아래 `runtime-config.json`이 없으면(또는 0바이트) 번들을 복사하거나 기본 JSON을 씁니다.
- **기본 userData (권장)**: Electron **AppData** (`%APPDATA%\choongman-pos-windows\` 또는 Omni는 `omnifoodtech-pos-windows\`) — **NSIS 업데이트해도 프린터·메인 설정이 유지**됩니다.
- 예전처럼 설치 폴더 `resources\choongman-pos-user-data`에 두려면 `WINDOWS_POS_USER_DATA=resources` (업데이트 시 초기화될 수 있어 **비권장**).
- 앱 시작 시 레거시 `resources\…` / 다른 브랜드 AppData / `%APPDATA%\cm-erp-pos-settings\runtime-config-backup.json` 에서 프린터·Partitions(메인 단말)를 **자동 복구**합니다.
- `CM_POS_USE_DEFAULT_USERDATA=1`은 하위 호환용(기본이 이미 AppData).
...
- (위 미지정·Windows 설치 본) **기본**: `%APPDATA%\<app-name>\` (Electron 기본) — 업데이트해도 유지
- `WINDOWS_POS_USER_DATA=resources` = 예전 `%ProgramFiles%\…\resources\choongman-pos-user-data\` (업데이트 시 초기화될 수 있음)
- `CM_POS_USE_DEFAULT_USERDATA=1` / `WINDOWS_POS_USE_DEFAULT_USERDATA=1` = 하위 호환(기본과 동일하게 AppData)
...
- `posLiveBlankPollMs`: 영업 중 흰 화면 검사 간격(ms). 기본 **5000**. 연속 2회 비어 있으면 **온라인**일 때는 Clear Cache 후 새로고침, **오프라인**일 때는 캐시로 다시 연다(캐시를 지우지 않음). 3분에 3번 넘으면 `offline.html`(터치 가능한 다시 시도)로 떨어진다. Windows POS는 서비스워커로 오프라인 셸을 유지한다. 배포 직후 JS 섞임은 CacheFirst(해시 JS)와 오프라인 시 캐시 비우기 금지로 막는다.
