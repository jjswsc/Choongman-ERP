...
- `posLiveBlankPollMs`: 영업 중 흰 화면 검사 간격(ms). 기본 **5000**. 연속 2회 비어 있으면 직원이 누른 **Clear Cache**와 같이 Electron 캐시를 지운 뒤 새로고침한다. 3분에 3번 넘으면 `offline.html`(터치 가능한 다시 시도)로 떨어진다. Windows POS는 서비스워커를 쓰지 않는다(배포 직후 JS 섞임 방지). 오프라인은 디스크 캐시와 `offline.html`.
