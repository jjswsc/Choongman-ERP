...
describe('expense-accrual-bank-link-candidates', () => {
  it('builds two flat date-window filters (no nested or=and)', () => {
    const filters = buildExpenseAccrualBankLinkDateFilters('2026-08-07', 14)
    expect(filters).toHaveLength(2)
    expect(filters[0]).toContain('status=in.(planned,approved,partial,paid,done)')
    expect(filters[0]).toContain('expense_date=gte.2026-07-24')
    expect(filters[0]).toContain('expense_date=lte.2026-08-21')
    expect(filters[1]).toContain('due_date=gte.2026-07-24')
    expect(filters[1]).toContain('due_date=lte.2026-08-21')
  ...
  it('builds amount filters for gross and WHT headroom', () => {
    ...
    expect(filters[1]).toMatch(/amount=lte\.2315/)
    for (const f of filters) {
      expect(f).not.toContain('or=(')
    }
  })
