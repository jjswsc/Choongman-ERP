...
export async function savePurchaseOrder(params: {
  ...
  /** FlowAccount 등 외부 문서와 맞출 소계·VAT·합계 수동 보정. null이면 기존 보정 해제(수정 저장) */
  moneyOverride?: { subtotal: number; vat: number; total: number } | null
  /** Draft PO 수정 — 있으면 해당 초안만 갱신(번호 유지) */
  poId?: number
}) {
