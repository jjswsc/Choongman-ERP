...
describe('findLayoutInvoiceNo', () => {
  ...
  it('RFT+일월연도가 길면 서기연도부터만 남긴다', () => {
    const got = findLayoutInvoiceNo(
      page([line(400, [['เลขที่เอกสาร', 1500], ['RFTKBKO27082026000023577', 1750]])])
    )
    expect(got?.value).toBe('2026000023577')
  })

  it('가시콘 입금통지 370…W 번호는 D머리·월접미를 떼고 본문만 남긴다', () => {
    const got = findLayoutInvoiceNo(
      page([line(400, [['เลขที่เอกสาร', 1500], ['370290826W02075', 1750]])])
    )
    expect(got?.value).toBe('370290826W02075')
  })

  it('구매자 세금번호에서 0이 빠진 12자리는 문서번호가 아니다', () => {
    const got = findLayoutInvoiceNo(
      page([
        line(300, [['เลขที่', 1400], ['105568080622', 1700]]),
        line(400, [['เลขที่เอกสาร', 1400], ['370290826W02075', 1700]]),
      ])
    )
    expect(got?.value).toBe('370290826W02075')
  })
  ...
  it('OCR이 TRSPEFHMO21189195 로 뭉개도 둘째 줄과 이어 붙인다', () => {
    const got = findLayoutInvoiceNo(
      page([
        line(514, [['เลขที่', 1379], ['No.', 1459], ['TRSPEFHMO21189195', 1545]]),
        line(548, [['0821-001305', 1546]]),
        line(580, [['วันที่', 1400], ['21/08/2026', 1700]]),
      ])
    )
    expect(got?.value).toBe('TRSPEFHM00-00000-260821-001305')
  })
...
describe('findLayoutTaxIds', () => {
  ...
  it('배송지 라벨이 위에 있어도 판매자 세금번호를 집는다', () => {
    const got = findLayoutTaxIds(
      page([
        line(120, [['ที่อยู่ในการจัดส่งเอกสาร', 200]]),
        line(160, [['บริษัท เอเชีย คอมเมิร์ซ แอนด์ เทรด จำกัด', 200]]),
        line(400, [['เลขประจำตัวผู้เสียภาษี', 200], ['0105553045044', 800]]),
        line(700, [['ที่อยู่ตามภาษีมูลค่าเพิ่ม', 200]]),
        line(740, [['เลขประจำตัวผู้เสียภาษี', 200], ['0105568080622', 800]]),
      ]),
      '0105568080622'
    )
    expect(got.seller?.value).toBe('0105553045044')
    expect(got.buyer?.value).toBe('0105568080622')
  })
