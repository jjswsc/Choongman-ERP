/**
 * 홀 테이블 **이동(move)** · **합석(merge)** · **포장→테이블(takeout_to_table)** 비즈니스 규칙 (서버·문서 단일 기준)
 *
 * ## 이동(move)
 ...
 *
 * ## 포장→테이블(takeout_to_table)
 * - `order_type`이 포장(takeout)인 주문만. 배달은 불가.
 * - 상태가 완료·취소·환불이 아닐 것.
 * - 목적지 `table_name`에 **다른 활성 홀 주문**이 없을 것(빈 테이블만). 이미 열린 테이블이면 합석 사용.
 * - DB: `order_type` → `dine_in`, `table_name` → 대상 테이블명.
 * - 미결제(결제 합계 0)이고 `packaging_fee` > 0 이면 포장비를 0으로 두고 `total`에서 차감.
 * - 결제 반영된 주문은 금액 필드 유지(유형·테이블명만 변경).
 * - 품목 `orderType`이 takeout이면 dine-in으로 맞춤.
 *
 * ## 합석(merge)
