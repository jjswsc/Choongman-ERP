...
import { ERP_POS_CATALOG_MENUS_CACHE_KEY } from '@/lib/offline/pos-catalog-offline'
 ...
 */
export function usePosMenusCatalogLiveRefresh(onUpdate: (menus: PosMenu[]) => void) {
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      if (ce.detail?.cacheKey !== ERP_POS_CATALOG_MENUS_CACHE_KEY) return
