...
const POS_INTERNAL_MEMO_STAMP_PREFIX =
  /^\[(?:PAY_CORRECT|MEMBER_ATTACH_AFTER_PAY|ORDER_(?:CANCELLED|REFUNDED|MERGED|MERGE_KEEP))\s+[^\]]*\]\s*(.*)$/i

/** 동일 스탬프가 한 줄 안에 여러 번 붙은 경우 */
const POS_INTERNAL_MEMO_STAMP_INLINE =
  /\[(?:PAY_CORRECT|MEMBER_ATTACH_AFTER_PAY|ORDER_(?:CANCELLED|REFUNDED|MERGED|MERGE_KEEP))\s+[^\]]*\]/gi
...
function stripInternalStampsFromTaxInvoicePayload(payload: string): string {
  ...
  if (!text) return ''
  text = text.replace(/\r?\n\[(?:PAY_CORRECT|MEMBER_ATTACH_AFTER_PAY|ORDER_(?:CANCELLED|REFUNDED|MERGED|MERGE_KEEP))\s+[^\]]*\][^\r\n]*/gi, '')
