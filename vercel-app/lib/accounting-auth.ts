import { isAccountingRole, isFranchiseeRole, isManagerRole, isOfficeRole, isOfficeStore } from '@/lib/permissions'
 ...
 */
export function canManageAccountingCompliance(role: string, userStore?: string): boolean {
  const normalizedStore = String(userStore || "").trim()
  const isOfficeLikeStore = isOfficeStore(normalizedStore) || isHeadOfficeLikeStoreName(normalizedStore)
  return (
    isOfficeRole(role) ||
...
export function assertCanManageAccountingCompliance(role: string, userStore?: string): void {
  if (!canManageAccountingCompliance(role, userStore)) {
