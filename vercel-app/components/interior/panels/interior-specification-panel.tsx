...
export function InteriorSpecificationPanel({ projectId }: { projectId: string }) {
  ...
  const handleSave = async () => {
    if (!editing || !editing.description?.trim()) {
      await appAlert(t("interiorSpecificationRequired"))
      return
    }
    try {
      ...
      <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8 space-y-4">
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold">{t("interiorSpecification")}</h2>
          </div>
        ...
        {editing && (
          <div className="rounded-lg border bg-card p-4 space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <div>
                <label className="text-xs text-muted-foreground">{t("interiorLocation")}</label>
        ...
        <div className="rounded-lg border bg-card">
          {loading ? (
            ...
            <div className="py-12 text-center text-sm text-muted-foreground">{t("interiorSpecificationEmpty")}</div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
