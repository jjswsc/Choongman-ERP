...
export function MemberPortalHomeMonthlyPromos({
  ...
  const promos = React.useMemo(
    () => listMemberPortalHomePromosForMonth(contentItems, yearMonth, monthRange),
    [contentItems, yearMonth, monthRange]
  )
    ...
    <GlassCard soft className="overflow-hidden p-0">
      <div className="flex items-center justify-between gap-2 border-b border-white/8 px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-sm font-semibold text-white">{t("homePromoTitle")}</p>
            {isCurrentMonth ? (
              <p className="text-[11px] text-white/45">{t("homePromoThisMonth")}</p>
            ) : null}
          </div>
        </div>
      ...
      {promos.length === 0 ? (
        <p className="px-4 py-8 text-center text-sm text-white/50">{t("homePromoEmpty")}</p>
      ) : (
            ...
            {promos.map((item) => (
              ...
              >
                {item.imageUrl ? (
                  <img
                    src={item.imageUrl}
                    alt={item.title || t("homePromoTitle")}
