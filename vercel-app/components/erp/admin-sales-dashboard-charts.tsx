...
const pieChartLegendStyle: React.CSSProperties = {
  fontSize: 11,
  paddingTop: 8,
  maxWidth: "100%",
}

/** 폴드·태블릿: 섹션 실제 폭이 충분할 때만 파이+표를 가로 배치 (뷰포트 sm이 아니라 컨테이너) */
const pieWithTableGridCn = "grid min-w-0 gap-4 @xl:grid-cols-2"
...
export function AdminSalesDashboardCharts({
  ...
  return (
    <section className="min-w-0 max-w-full space-y-4 overflow-x-auto overscroll-x-contain rounded-xl border border-border/70 bg-card p-4 shadow-sm [-webkit-overflow-scrolling:touch]">
      ...
      {hourRows.some((r) => r.sales > 0) ? (
        <div className="min-w-0">
          ...
          <div className="h-[200px] min-w-0 overflow-x-auto overscroll-x-contain [-webkit-overflow-scrolling:touch]">
      ...
      <div className={`grid min-w-0 gap-6 ${showStoreTable ? "xl:grid-cols-2" : ""}`}>
        {showStoreTable ? (
          <div className="min-w-0 xl:col-span-2">
            ...
            <div className="space-y-2 md:hidden">
              {storeChartRows.map((r) => (
                <div key={r.storeName} className="rounded-lg border bg-card px-3 py-2.5 text-xs">
                  <p className="mb-1.5 font-medium">{r.storeDisplayName}</p>
                  <dl className="grid grid-cols-2 gap-x-3 gap-y-1 tabular-nums">
                    <div className="flex justify-between gap-2">
                      <dt className="text-muted-foreground">{tr("salesAmountKindDineIn", "홀")}</dt>
                      <dd>{formatSalesAmount(r.dineInTotal)}</dd>
                    </div>
                    <div className="flex justify-between gap-2">
                      <dt className="text-muted-foreground">{tr("salesAmountKindDelivery", "배달")}</dt>
                      <dd>{formatSalesAmount(r.deliveryTotal)}</dd>
                    </div>
                    <div className="flex justify-between gap-2">
                      <dt className="text-muted-foreground">{tr("salesAmountKindTakeout", "포장")}</dt>
                      <dd>{formatSalesAmount(r.takeoutTotal)}</dd>
                    </div>
                    <div className="flex justify-between gap-2">
                      <dt className="text-muted-foreground">
                        {tr("mobileStoreSalesUnpaidTableTotal", "미결제 테이블")}
                      </dt>
                      <dd>{formatSalesAmount(r.tableTotal)}</dd>
                    </div>
                    <div className="col-span-2 flex justify-between gap-2 border-t pt-1 font-semibold">
                      <dt>{tr("salesTotalLabel", "합계")}</dt>
                      <dd>{formatSalesAmount(r.sales)}</dd>
                    </div>
                  </dl>
                </div>
              ))}
              <div className="rounded-lg border bg-muted/40 px-3 py-2.5 text-xs font-semibold">
                <dl className="grid grid-cols-2 gap-x-3 gap-y-1 tabular-nums">
                  <div className="flex justify-between gap-2">
                    <dt className="font-medium">{tr("salesAmountKindDineIn", "홀")}</dt>
                    <dd>{formatSalesAmount(storeTableTotals.dineIn)}</dd>
                  </div>
                  <div className="flex justify-between gap-2">
                    <dt className="font-medium">{tr("salesAmountKindDelivery", "배달")}</dt>
                    <dd>{formatSalesAmount(storeTableTotals.delivery)}</dd>
                  </div>
                  <div className="flex justify-between gap-2">
                    <dt className="font-medium">{tr("salesAmountKindTakeout", "포장")}</dt>
                    <dd>{formatSalesAmount(storeTableTotals.takeout)}</dd>
                  </div>
                  <div className="flex justify-between gap-2">
                    <dt className="font-medium">{tr("mobileStoreSalesUnpaidTableTotal", "미결제 테이블")}</dt>
                    <dd>{formatSalesAmount(storeTableTotals.table)}</dd>
                  </div>
                  <div className="col-span-2 flex justify-between gap-2 border-t pt-1">
                    <dt>{tr("salesTotalLabel", "합계")}</dt>
                    <dd>{formatSalesAmount(storeTableTotals.sales)}</dd>
                  </div>
                </dl>
              </div>
            </div>
            <div className="hidden min-w-0 md:block">
              <AdminTableScroll className="rounded-lg border" lockViewport={false}>
        ...
        {channelChartRows.length > 0 ? (
          <div className="@container min-w-0">
            ...
            <div className={pieWithTableGridCn}>
              <div className="h-[220px] min-w-0 w-full overflow-hidden">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                    ...
                    />
                    <Legend verticalAlign="bottom" wrapperStyle={pieChartLegendStyle} />
                  </PieChart>
                </ResponsiveContainer>
              </div>
