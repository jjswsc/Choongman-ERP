...
import { getNoticeOptions, getHrPolicyReaderStats, type NoticeReaderStatsRow } from "@/lib/api-client"
import {
  Select,
  SelectContent,
  SelectItem,
...
export function HrPolicyReaderStatsDialog({ open, onOpenChange }: Props) {
  const { auth } = useAuth()
  const { lang } = useLang()
  const t = useT(lang)
  ...
  const [statsPolicyCount, setStatsPolicyCount] = React.useState(0)
  ...
  const runReaderStats = React.useCallback(async () => {
    ...
    try {
      const res = await getHrPolicyReaderStats({
      ...
      if (!res.success && res.message) {
        ...
        setStatsPolicyCount(0)
        return
      }
      setStatsItems(res.items)
      setStatsTruncated(res.truncated)
      setStatsPolicyCount(res.policyInRange)
    } catch {
      setStatsItems([])
      setStatsTruncated(false)
      setStatsPolicyCount(0)
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-3xl max-h-[85vh] overflow-hidden flex flex-col gap-3 p-4 sm:p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
          ...
          <div className="sm:col-span-2">
            ...
            <Input
              ...
              className="h-9 text-xs w-full sm:max-w-[120px]"
            />
          </div>
        </div>
        <Button
        ...
        {statsPolicyCount > 0 && !statsLoading && (
          <p className="text-[10px] text-muted-foreground">
            {t("hrPolicyCountPrefix")} {statsPolicyCount} {t("hrPolicyCountSuffix")}
          </p>
        )}
