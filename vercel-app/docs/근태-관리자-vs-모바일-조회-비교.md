...
## 8. 최근 UI/동작 정리 (관리자·모바일 공통)

| 항목 | 내용 |
|------|------|
| **모바일 근태 승인** | "승인대기" / "미기록" 탭 제거 → **단일 테이블**로 통합. 관리자와 동일한 컬럼(일자, 이름, 출근, 퇴근, 휴게, 실제, 계획, 차이, 지각/연장, 조정, 상태, 퇴근·연장 승인). 매장은 상단에서만 선택(테이블에 매장 컬럼 없음). **상태 필터** 드롭다운 제공(전체, 정상 제외, 미기록, 연장, 지각 등). |
| **상태 필터** | 관리자·모바일 모두 **상태** 검색창: 전체, 정상 제외, 미기록, 연장, 지각 + 조회 결과에서 나온 그 외 상태. |
| **조퇴** | 계획보다 **1분이라도 적게 근무**(차이 음수)하면 "조퇴"로 표시. 조정 칸에 조퇴 인정 분 입력 후 승인 가능. |
| **차이 컬럼** | "M" 제거. 숫자만 표시(예: +6, -6). |
| **설명(근태 관리)** | 관리자 페이지 "설명" 탭 및 i18n `att_help_*`: 조정(지각/조퇴/연장), 차이 표시, 상태(조퇴 기준·상태 필터) 설명 반영됨. |
