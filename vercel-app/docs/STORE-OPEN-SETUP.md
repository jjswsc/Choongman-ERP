...
### 2-5. 오프라인 모드 운영 주의사항 (중요)

#### A) `dev` 와 `start`/`prod` 동작 차이

- `npm run dev`: Service Worker(PWA) 캐시가 기본적으로 비활성화될 수 있어, **오프라인 메뉴/테이블 검증이 실제 운영과 다르게 보일 수 있음**
- `npm run build && npm run start`: 운영과 같은 캐시 경로 검증 가능 (권장)
- 현장 리허설은 가능하면 `start` 모드로 수행

#### B) Origin(접속 주소) 1개로 통일

- `http://localhost:3000` 과 `http://192.168.x.x:3000` 은 **서로 다른 저장소(IndexedDB/localStorage)** 로 취급됨
- 테스트/실사용 동안 주소를 섞어 쓰면 오프라인 캐시가 비어 보일 수 있음
- 규칙: **기기별로 하나의 주소만 고정 사용** (`localhost` 또는 `IP` 중 하나)

#### C) "오프라인용 받기" 실행 타이밍

- 오픈 전 인터넷이 정상일 때 1회 실행
- 메뉴 변경/테이블 변경/프린터 설정 변경 후 즉시 재실행
- 실행 후 POS 상단에서 `동기화 대기 건수`와 최근 캐시 시각을 확인
...
| 7 | POS 접속 Origin 1개 고정 (`localhost`/`IP` 혼용 금지) | 캐시 분리 방지 |
| 8 | 오픈 전 "오프라인용 받기" 실행 + 변경 시 재실행 | 메뉴/테이블 캐시 최신화 |
| 9 | 월 1회 오프라인 리허설 (`build/start` 모드) | SW/PWA 경로 점검 |
...
- **돈통 로컬 브리지(자동 열림):** `vercel-app/docs/POS-CASH-DRAWER-BRIDGE.md`
