...
export function usePosKbankPayment(params: UsePosKbankPaymentParams): UsePosKbankPaymentReturn {
  ...
  const clearKbankQrSession = useCallback(() => {
    ...
    kbankCcInquiryTriggeredRef.current = ''
  }, [purgeKbankPendingFinalize, clearKbankQrFromLinkpos, setCustomerDisplayPaymentMessage, currentStoreId])
  ...
  const runKbankQrPaymentIfNeeded = useCallback(
    async (
      ...
      if (kbankGenerateInFlightRef.current) {
        ...
        return { ok: false, message: 'kbank_generate_in_flight' }
      }
      ...
      if (requestedQrType !== 'CREDIT_CARD') {
        ...
        if (isKbankQrSessionExpired(kbankQrSessionStartedAtRef.current)) {
          ...
          return { ok: false, message: 'kbank_qr_expired' }
        }
        if (!isKbankApiPaused()) {
