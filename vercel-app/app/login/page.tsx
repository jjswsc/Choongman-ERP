...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('')
    try {
      const res = await loginCheck({ store, name: user, pw, isAdminPage: false })
  ...
  const labels = {
    ...
    th: { selectStore: 'เลือกสาขา', selectName: 'เลือกชื่อ', pinPlaceholder: 'รหัสผ่าน (PIN)', login: 'เข้าสู่ระบบ', loggingIn: 'กำลังเข้าสู่ระบบ...', changePw: 'เปลี่ยนรหัสผ่าน', pwCurrent: 'รหัสปัจจุบัน', pwNew: 'รหัสใหม่', pwNewConfirm: 'ยืนยันรหัสใหม่', pwChangeBtn: 'เปลี่ยน' },
    mm: { selectStore: 'ဆိုင်ရွေးပါ', selectName: 'အမည်ရွေးပါ', pinPlaceholder: 'လျှို့ဝှက်နံပါတ် (PIN)', login: 'ဝင်ရောက်မည်', loggingIn: 'ဝင်နေသည်...', changePw: 'လျှို့ဝှက်နံပါတ်ပြောင်းမည်', pwCurrent: 'လက်ရှိလျှို့ဝှက်နံပါတ်', pwNew: 'လျှို့ဝှက်နံပါတ်အသစ်', pwNewConfirm: 'အသစ်ထပ်ရိုက်ပါ', pwChangeBtn: 'ပြောင်းမည်' },
    la: { selectStore: 'ເລືອກສາຂາ', selectName: 'ເລືອກຊື່', pinPlaceholder: 'ລະຫັດ (PIN)', login: 'ເຂົ້າສູ່ລະບົບ', loggingIn: 'ກຳລັງເຂົ້າສູ່ລະບົບ...', changePw: 'ປ່ຽນລະຫັດຜ່ານ', pwCurrent: 'ລະຫັດປັດຈຸບັນ', pwNew: 'ລະຫັດໃໝ່', pwNewConfirm: 'ຢືນຢັນລະຫັດໃໝ່', pwChangeBtn: 'ປ່ຽນ' },
  }
  ...
  return (
    <div className="login-page">
          ...
          <form onSubmit={handleSubmit}>
            ...
            {noStores && (
              <p className="-mt-2 mb-2 text-xs text-amber-400">
                {lang === 'ko' ? '매장 목록을 불러올 수 없습니다. vercel-app/.env에 SUPABASE_URL, SUPABASE_ANON_KEY를 설정해 주세요.' : 'Cannot load store list. Set SUPABASE_URL and SUPABASE_ANON_KEY in vercel-app/.env'}
              </p>
            )}
      ...
      {pwModalOpen && (
        <div
          className="login-pw-modal"
          onClick={() => setPwModalOpen(false)}
        >
          <div
            className="login-pw-modal-inner"
          ...
          >
            ...
            <form onSubmit={handlePwChange}>
              ...
              />
              {pwError && (
                <p className="mb-3 text-sm text-red-400">{pwError}</p>
              )}
              <button
                type="submit"
