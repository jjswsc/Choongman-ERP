...
import {
  getPosCostAnalysisAudit,
  getPosMenuCostAnalysis,
  type PosCostAnalysisAuditRow,
        ...
        <div className="rounded-xl border bg-card overflow-hidden">
          ...
          <TabsContent value="audit" className={cn(adminTabsContentCn, "space-y-4")}>
            <div className="flex flex-wrap items-center gap-3">
              ...
              >
                <SelectTrigger className="h-9 w-32 text-xs">
                  <SelectValue placeholder={t("date") || "기간"} />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="today">{t("today") || "오늘"}</SelectItem>
                  <SelectItem value="7d">{t("last7Days") || "최근 7일"}</SelectItem>
                  <SelectItem value="30d">{t("last30Days") || "최근 30일"}</SelectItem>
                  <SelectItem value="custom">{t("custom") || "직접 선택"}</SelectItem>
                </SelectContent>
              </Select>
              <Input
                type="date"
              ...
              >
                <SelectTrigger className="h-9 w-32 text-xs">
                  <SelectValue placeholder={t("status") || "작업"} />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">{t("all") || "전체"}</SelectItem>
                  <SelectItem value="insert">{t("create") || "추가"}</SelectItem>
                  <SelectItem value="update">{t("edit") || "수정"}</SelectItem>
                  <SelectItem value="delete">{t("delete") || "삭제"}</SelectItem>
                </SelectContent>
              </Select>
              <Select value={auditActorFilter} onValueChange={setAuditActorFilter}>
                <SelectTrigger className="h-9 w-40 text-xs">
                  <SelectValue placeholder={t("manager") || "담당자"} />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">{t("all") || "전체"}</SelectItem>
              ...
              <div className="relative flex-1 min-w-[220px] max-w-sm">
                ...
                <Input
                  ...
                  placeholder={t("posCostAuditSearchPh") || "메뉴·재료·담당자 검색"}
              ...
              >
                ...
                {auditLoading ? (t("loading") || "조회 중...") : (t("posCostBtnQuery") || "조회")}
              </Button>
            </div>
            ...
            {auditLoading ? (
              ...
              <div className="rounded-xl border bg-muted/30 px-6 py-12 text-center text-sm text-muted-foreground">
                {t("posCostAuditHint") || "원가분석 수정 이력을 확인하려면 [조회]를 누르세요."}
              </div>
            ) : (
              <div className="rounded-xl border bg-card overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b bg-muted/50">
                        <th className="px-3 py-2 text-left text-xs font-semibold">{t("dateTime") || "시간"}</th>
                        <th className="px-3 py-2 text-left text-xs font-semibold">{t("posMenuCode") || "메뉴"}</th>
                        <th className="px-3 py-2 text-left text-xs font-semibold">{t("posMenuIngredients") || "재료"}</th>
                        <th className="px-3 py-2 text-right text-xs font-semibold">{t("posCostQty") || "수량"}</th>
                        <th className="px-3 py-2 text-right text-xs font-semibold">{t("posIngredientLoss") || "로스"}</th>
                        <th className="px-3 py-2 text-left text-xs font-semibold">{t("posCostIngredientType") || "타입"}</th>
                        <th className="px-3 py-2 text-left text-xs font-semibold">{t("status") || "작업"}</th>
                        <th className="px-3 py-2 text-left text-xs font-semibold">{t("manager") || "담당자"}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {filteredAuditRows.map((r) => (
                        <tr key={r.id} className="border-b last:border-b-0">
                          ...
                          <td className="px-3 py-2 text-right tabular-nums">{(Number(r.quantity) || 0).toFixed(2)}</td>
                          <td className="px-3 py-2 text-right tabular-nums">
                            {(Number(r.lossRate) || 0) > 0 ? `${(Number(r.lossRate) || 0).toFixed(2)}%` : "—"}
                          </td>
                          <td className="px-3 py-2 text-xs">
                            {!r.ingredientType
                              ? "-"
                              : r.ingredientType === "packaging"
                                ? (t("posCostTypePackaging") || "포장")
                                : (t("posCostTypeFood") || "음식")}
                          </td>
                          <td className="px-3 py-2 text-xs">
                            {r.actionType === "insert"
                              ? (t("create") || "추가")
