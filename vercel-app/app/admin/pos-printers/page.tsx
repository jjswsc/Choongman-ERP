...
import { canPickPosTerminalStore, canAccessPosPrinters, hasOfficeStaffScope } from "@/lib/permissions"
import {
...
export default function PosPrintersPage() {
  ...
  const [receiptShowBizAddress, setReceiptShowBizAddress] = React.useState(false)
  ...
  const loadDefaultMembershipQr = React.useCallback(() => {
    setReceiptMembershipQrLinkUrl(POS_MEMBERSHIP_POINTS_MANUAL_QR_LINK)
    setReceiptMembershipQrImageUrl(POS_MEMBERSHIP_POINTS_MANUAL_QR_IMAGE_PATH)
    setReceiptMembershipQrText((prev) => prev.trim() || POS_MEMBERSHIP_POINTS_MANUAL_QR_TEXT_DEFAULT)
    setReceiptShowMembershipQr(true)
  }, [])

  const handleApplyMembershipQrToAllStores = React.useCallback(async () => {
    if (!canApplyMembershipQrAll) {
      await appAlert(tr("posReceiptMembershipQrApplyAllOfficeOnly", "본사 권한에서만 전 매장 일괄 적용할 수 있습니다."))
      return
    }
    ...
    if (!ok) return
    setMembershipQrApplyingAll(true)
    try {
      loadDefaultMembershipQr()
      const res = await applyMembershipQrToAllStores({
        storeCodes: stores,
        linkUrl: POS_MEMBERSHIP_POINTS_MANUAL_QR_LINK,
        imageUrl: POS_MEMBERSHIP_POINTS_MANUAL_QR_IMAGE_PATH,
        text: receiptMembershipQrText.trim() || POS_MEMBERSHIP_POINTS_MANUAL_QR_TEXT_DEFAULT,
        show: true,
      })
      if (!res?.success) {
        await appAlert(res?.message || tr("posReceiptMembershipQrApplyAllFail", "전 매장 QR 일괄 적용에 실패했습니다."))
        return
      }
      await appAlert(
        (tr("posReceiptMembershipQrApplyAllDone", "{{n}}개 매장에 멤버십 QR을 적용했습니다. POS를 새로고침하면 영수증에 반영됩니다.") || "").replace(
          "{{n}}",
          String(Math.max(n, Number(res.upsertedStores || 0)))
        )
      )
    } catch (e) {
      await appAlert(e instanceof Error ? e.message : tr("posReceiptMembershipQrApplyAllFail", "전 매장 QR 일괄 적용에 실패했습니다."))
    } finally {
      setMembershipQrApplyingAll(false)
    }
  }, [canApplyMembershipQrAll, stores, tr, loadDefaultMembershipQr, receiptMembershipQrText])
  ...
  const applyFromPosSettings = React.useCallback((settings: PosPrinterSettings) => {
    ...
    setEscPosCutAfterHallOrderHtml(settings.escPosCutAfterHallOrderHtml !== false)
    setEscPosCutAfterPaymentReceiptHtml(settings.escPosCutAfterPaymentReceiptHtml !== false)
    ...
    setReceiptShowBizAddress(Boolean(settings.receiptShowBizAddress))
      ...
      .map((it, idx) => {
        ...
        const promoHtml =
          Array.isArray(it.promoLines) && it.promoLines.length > 0
            ...
            : ""
        return `<div class="receipt-row"><span>${it.qty}x ${escapeHtml(it.name)}</span><span>${formatBahtNum(
          it.qty * it.price
        )}</span></div>${promoHtml}${
          itemBarcodeUrl
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .receipt-meta-row { display: grid; grid-template-columns: minmax(0, max-content) minmax(0, 1fr); column-gap: 3mm; align-items: start; margin: 3px 0; padding-right: 0.4mm; }
        ...
        <body>
          ...
          ${lines}
          <div class="receipt-divider"></div>
          <div class="receipt-row"><span>${escapeHtml(t("posSubtotal") || "소계")}</span><span>${formatBahtNum(previewData.subtotal)}</span></div>
          <div class="receipt-row"><span>${escapeHtml(t("posDiscount") || "할인")}</span><span>-${formatBahtNum(previewData.discount)}</span></div>
          <div class="receipt-row"><span>${escapeHtml(t("posDeliveryFee") || "배달 수수료")}</span><span>+${formatBahtNum(previewData.delivery)}</span></div>
          <div class="receipt-row"><span>${escapeHtml(t("posPackagingFee") || "포장 수수료")}</span><span>+${formatBahtNum(previewData.packaging)}</span></div>
          <div class="receipt-divider-strong"></div>
          <div class="receipt-total">
            <div class="receipt-row"><span>${escapeHtml(t("posTotal") || "합계")}</span><span>${formatBahtNum(previewData.total)}</span></div>
          </div>
          <div class="receipt-divider"></div>
          ...
          ${buildPaymentReceiptMemberFooterHtml({
            ...
            showMembershipQr: Boolean(receiptShowMembershipQr && qrSrc),
            membershipQrSrc: String(qrSrc || ""),
            membershipQrText: qrCaption,
            tr,
            esc: escapeHtml,
          })}
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <TabsContent value="receipt-design" className={cn(adminTabsContentCn, "space-y-4")}>
              ...
              <div className="rounded-lg border p-4 space-y-3">
                ...
                <div className="rounded-md border p-3 space-y-2">
                  <p className="text-xs font-medium">{tr("posReceiptMembershipQrTitle", "멤버십 QR (신규 가입)")}</p>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {canApplyMembershipQrAll ? (
                      <Button
                        type="button"
                        size="sm"
                        disabled={membershipQrApplyingAll}
                        onClick={() => void handleApplyMembershipQrToAllStores()}
                      >
                        {membershipQrApplyingAll
                          ? t("loading")
                          : tr("posReceiptMembershipQrApplyAll", "전 매장 일괄 적용")}
                      </Button>
                    ) : null}
                  </div>
                  <Input
            ...
            <TabsContent value="business" className={cn(adminTabsContentCn, "space-y-4")}>
              <div className="rounded-lg border p-4 space-y-3">
                ...
                <div>
                  ...
                  <div className="mt-2">
                    <ToggleRow
                      label={tr("posReceiptShowBizAddressLabel", "손님 영수증에 사업장 주소 인쇄")}
                      value={receiptShowBizAddress}
                      onChange={setReceiptShowBizAddress}
                      t={t}
                    />
                    <p className="mt-1 text-[11px] text-muted-foreground">
                      {tr(
                        "posReceiptShowBizAddressHint",
                        "기본은 끔(용지 절약). Tax Invoice 요청 시에는 별도 블록에 주소를 반드시 입력·인쇄합니다."
                      )}
                    </p>
                  </div>
                </div>
          ...
          <div className="max-h-[70vh] overflow-auto rounded-md border bg-muted/20 p-4">
            ...
            >
                <div className="font-mono text-xs">
                  ...
                  {previewData.items.map((it, idx) => (
                    <div key={`pv-receipt-${idx}`} className="my-1">
                      <div className="flex items-center justify-between">
                        ...
                        <span>{formatBahtNum(it.qty * it.price)}</span>
                      </div>
                      {Array.isArray(it.promoLines) && it.promoLines.length > 0
                  ...
                  <div className="my-2 border-t border-dashed border-black" />
                  <div className="my-1 flex items-center justify-between"><span>{t("posSubtotal") || "소계"}</span><span>{formatBahtNum(previewData.subtotal)}</span></div>
                  <div className="my-1 flex items-center justify-between"><span>{t("posDiscount") || "할인"}</span><span>-{formatBahtNum(previewData.discount)}</span></div>
                  <div className="my-1 flex items-center justify-between"><span>{t("posDeliveryFee") || "배달 수수료"}</span><span>+{formatBahtNum(previewData.delivery)}</span></div>
                  <div className="my-1 flex items-center justify-between"><span>{t("posPackagingFee") || "포장 수수료"}</span><span>+{formatBahtNum(previewData.packaging)}</span></div>
                  <div className="my-2 border-t-2 border-black" />
                  <div className="font-bold">
                    <div className="flex items-center justify-between"><span>{t("posTotal") || "합계"}</span><span>{formatBahtNum(previewData.total)}</span></div>
                  </div>
                  <div className="my-2 border-t border-dashed border-black" />
                  {receiptBarcode ? (
                    ...
                    <div className="flex gap-2 text-[10px] font-bold text-black">
                      {receiptShowMembershipQr && membershipQrPreviewSrc ? (
                        <div className="w-[38%] shrink-0 text-center">
                          ...
                          />
                          <div className="mx-auto mt-1 max-w-[78px] rounded-sm bg-black px-1 py-0.5 text-[8px] font-extrabold leading-tight text-white">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="mb-0.5 text-[11px] font-extrabold">
                        ...
                        <div>
                          {tr("posReceiptMemberPointsEarn", "คะแนนที่จะได้รับ")}: +4
                        </div>
