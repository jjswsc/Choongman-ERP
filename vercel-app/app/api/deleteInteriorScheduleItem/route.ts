...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const rows = (await supabaseSelectFilter('interior_schedule_items', `id=eq.${id}`, {
      limit: 1,
    ...
    await supabaseDeleteByFilter('interior_schedule_items', `id=eq.${id}`)
    return NextResponse.json({ success: true, message: '삭제되었습니다.' }, { headers })
  } catch (e) {
    console.error('deleteInteriorScheduleItem:', e)
