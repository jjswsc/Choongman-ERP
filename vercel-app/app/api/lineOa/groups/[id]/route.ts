...
import {
  deleteLineOaGroup,
  fetchLineOaGroupById,
  getLineOaGroupCredentials,
  parseLineOaGroupId,
  parsePatchLineOaGroupBody,
  patchLineOaGroup,
...
export async function GET(_request: NextRequest, context: RouteContext) {
  ...
  try {
    const upstream = await fetchLineOaGroupById(parsedId.id)
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  try {
    const upstream = await patchLineOaGroup(parsedId.id, parsed.body as Record<string, unknown>)
    const text = await upstream.text()
    let body: unknown = text
    try {
      body = text ? JSON.parse(text) : null
...
export async function DELETE(_request: NextRequest, context: RouteContext) {
  ...
  try {
    const upstream = await deleteLineOaGroup(parsedId.id)
