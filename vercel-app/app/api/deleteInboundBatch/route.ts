...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const batchRows = (await supabaseSelectFilter('inbound_batches', `id=eq.${batchId}`, { limit: 1 })) as { id?: number }[]
    if (!batchRows?.length) {
    ...
    await supabaseDeleteByFilter('stock_logs', `inbound_batch_id=eq.${batchId}`)

    // 3. inbound_batches 삭제
    await supabaseDeleteByFilter('inbound_batches', `id=eq.${batchId}`)
