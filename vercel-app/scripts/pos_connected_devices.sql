...
create table if not exists public.pos_connected_devices (
  ...
  role text not null default 'order' check (role in ('main', 'order', 'attendance_display')),
...
comment on column public.pos_connected_devices.role is 'main: 메인 POS, order: 주문 단말, attendance_display: 출퇴근 QR 표시 전용';
