/**
 * 단일 API 진입점 (Hobby 플랜 12개 함수 제한)
 * 호출: /api/handler?path=getLoginData 등
 * api/handler/index.js -> 경로 /api/handler
 */
const handlers = {
...
module.exports = async (req, res) => {
  let name = (req.query && req.query.path) || '';
  if (!name && req.url) {
    const pathname = req.url.split('?')[0];
    name = pathname.replace(/^\/api\/?/, '').split('/')[0];
  }
  if (!name) {
    res.setHeader('Access-Control-Allow-Origin', '*');
    return res.status(404).json({ error: 'API name required' });
  }
